<?php
    ...
    if (empty($_SESSION['user_id']) && isset($_POST['login_submission']))
    {
        ...
        if (!empty($username) && !empty($password))
        {
            ...
            $dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME)
                or trigger_error("Problem with connecting to the DB", E_USER_ERROR);
            
            $query = "SELECT id, username, password, admin FROM user WHERE username = ?";
            ...
            {
                echo "<h4><p class='text-dark'>An account does not exist for this username:"
                . "<span class='font-weight-bold text-danger'> ($username)</span>. "
    ...
    ?>        
    <h2 class="text-primary text-center p-2">Sign in to access the Steel Pan Database!</h2>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-4">
                        <input type="text" class="form-control" id="username"
                            name="username" placeholder="Enter a user name" required>
